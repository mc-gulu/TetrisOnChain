...
namespace IFix.Editor
{
    ...
    public class IFixEditor
    {
        ...
        public const string IFixCoreDLLPath = "./Assets/Plugins/SharedPlugins/IFix.Core.dll";
            ...
            string corePath = IFixCoreDLLPath)
        {
            ...
            foreach (var assembly in injectAssemblys)
            {
                GenPatch(assembly, string.Format("{0}/{1}.dll", outputDir, assembly),
                    IFixCoreDLLPath, string.Format("{0}{1}.patch.bytes", patchOutputDir, assembly));
            }
#else
        ...
        public static void GenPatch(string assembly, string assemblyCSharpPath
            ...
            string corePath = IFixCoreDLLPath, string patchPath = "Assembly-CSharp.patch.bytes")
        {
            ...
            try
            {
                foreach (var assembly in injectAssemblys)
                {
                    GenPatch(assembly, string.Format("./Library/ScriptAssemblies/{0}.dll", assembly),
                        IFixCoreDLLPath, string.Format("{0}.patch.bytes", assembly));
                }
            }
            catch (Exception e)
            {
