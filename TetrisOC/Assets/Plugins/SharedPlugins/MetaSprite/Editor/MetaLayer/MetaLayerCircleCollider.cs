...
namespace MetaSprite
{
    ...
    public class MetaLayerCircleCollider : MetaLayerProcessor
    {
        ...
        public override void Process(ImportContext ctx, Layer layer)
        {
            ...
            EditorCurveBinding
                bindingOffX = Binding(path, typeof(CircleCollider2D), "m_Offset.x"),
                bindingOffY = Binding(path, typeof(CircleCollider2D), "m_Offset.y"),
                bindingRadius = Binding(path, typeof(CircleCollider2D), "m_Radius"),
                bindingEnable = Binding(path, typeof(CircleCollider2D), "m_Enabled");
            ...
            foreach (var frameTag in ctx.file.frameTags)
            {
                ...
                AnimationCurve
                    ...
                    curveRadius = new AnimationCurve(),
                ...
                for (int f = frameTag.from; f <= frameTag.to; ++f)
                {
                    ...
                    if (enable)
                    {
                        ...
                        curveRadius.AddKey(t, rect.size.x);
                    }
                ...
                if (hasEnable)
                {
                    ...
                    AnimationUtility.SetEditorCurve(clip, bindingRadius, curveRadius);
